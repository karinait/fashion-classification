...
```bash
fashion-classifier$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
fashion-classifier$ sam local start-api
fashion-classifier$ curl http://localhost:3000/classify_digit
```
...
```bash
fashion-classifier$ sam logs -n InferenceFunction --stack-name "fashion-classifier" --tail
```
...
```bash
sam delete --stack-name "fashion-classifier"
```
